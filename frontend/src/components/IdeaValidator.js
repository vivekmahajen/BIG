...
export default function IdeaValidator({ sector, city, state, zip, country, onNavigate, prefill }) {
  ...
  function handleDownload() {
    if (!result) return;
    const verdictColor = result.score >= 70 ? '#10b981' : result.score >= 45 ? '#f59e0b' : result.score >= 20 ? '#f97316' : '#ef4444';
    const dimRows = (result.dimensions || []).map(d => {
      const c = d.score >= 65 ? '#10b981' : d.score >= 40 ? '#f59e0b' : '#ef4444';
      return `<tr><td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;">${d.name}</td>
        <td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;color:${c};font-weight:700;">${d.score}/100</td>
        <td style="padding:8px 12px;border-bottom:1px solid #e2e8f0;font-size:12px;color:#64748b;">${d.rationale || ''}</td></tr>`;
    }).join('');
    const movesList = (result.movesUpABand || []).map(m => `<li style="margin-bottom:6px;">${m}</li>`).join('');
    const w = window.open('', '_blank');
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"/>
<title>BIG Validation — ${idea.slice(0, 60)}</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  h2{font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#64748b;margin:22px 0 8px;border-bottom:2px solid #6366f1;padding-bottom:4px;}
  ...
  .brand{font-size:24px;font-weight:900;color:#6366f1;letter-spacing:-2px;}
  .hero{display:flex;justify-content:space-between;align-items:center;padding:16px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:16px;}
  .verdict{font-size:20px;font-weight:800;color:${verdictColor};}
  .score-circle{width:64px;height:64px;border-radius:50%;border:3px solid ${verdictColor};display:flex;flex-direction:column;align-items:center;justify-content:center;}
  .score-num{font-size:20px;font-weight:900;color:${verdictColor};}
  .score-max{font-size:10px;color:#94a3b8;}
  .bear{background:#fef2f2;border:1px solid #fecaca;border-radius:6px;padding:12px 16px;margin-bottom:12px;font-size:13px;}
  table{width:100%;border-collapse:collapse;font-size:12px;margin-bottom:16px;}
  th{text-align:left;padding:8px 12px;background:#f1f5f9;font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:#64748b;}
  ul{padding-left:20px;}
  .footer{margin-top:32px;font-size:10px;color:#94a3b8;border-top:1px solid #e2e8f0;padding-top:12px;}
  @media print{body{padding:20px;}}
</style></head><body>
<div class="header">
  <div><div class="brand">BIG</div><div style="font-size:11px;color:#64748b;">Business Idea Validation Report</div></div>
  <div style="text-align:right;font-size:11px;color:#64748b;">${new Date().toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})}</div>
</div>
<h1>${idea.slice(0, 100)}${idea.length > 100 ? '…' : ''}</h1>
<p style="color:#64748b;margin-bottom:16px;font-size:12px;">
  Customer: ${result.inputEcho?.inferredCustomer || 'N/A'} · Geography: ${result.inputEcho?.inferredGeography || 'N/A'}
</p>
<div class="hero">
  <div>
    <div class="verdict">${result.verdict}</div>
    <div style="font-size:12px;color:#64748b;margin-top:4px;max-width:480px;">${result.verdictReason || ''}</div>
    <div style="font-size:11px;color:#94a3b8;margin-top:6px;">Confidence: ${result.verdictConfidence}</div>
  </div>
  <div class="score-circle"><span class="score-num">${result.score}</span><span class="score-max">/100</span></div>
</div>
<h2>Strongest reason it may fail</h2>
<div class="bear">${result.bearCase || ''}</div>
<h2>Riskiest assumption to test next</h2>
<p style="margin-bottom:16px;">${result.riskiestAssumption || ''}</p>
<h2>Dimension scores</h2>
<table><thead><tr><th>Dimension</th><th>Score</th><th>Rationale</th></tr></thead><tbody>${dimRows}</tbody></table>
<h2>What would move it up a band</h2>
<ul style="margin-bottom:16px;">${movesList}</ul>
<div class="footer">Generated by BIG — Business Opportunity Intelligence · big-eosin.vercel.app · For informational purposes only.</div>
      ...
      {result && meta && (
        <div className={styles.result} aria-live="polite">
          ...
          <div className={styles.resultActions}>
            ...
            <button className={styles.newValidationBtn} onClick={handleDownload} style={{ background: 'linear-gradient(135deg,#6366f1,#818cf8)', color: '#fff', border: 'none' }}>⬇ Download PDF</button>
            <SaveButton
              cardData={cardData}
              state={state}
              city={city}
