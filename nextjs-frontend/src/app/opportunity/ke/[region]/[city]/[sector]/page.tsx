...
export async function generateMetadata({ params }: { params: Promise<{ region: string; city: string; sector: string }> }): Promise<Metadata> {
  ...
  const title = `${sector.label} Business Opportunities in ${city.city}, ${region.name} (Kenya) | BIG`;
  const description = `Discover the best ${sector.label.toLowerCase()} business opportunities in ${city.city}, ${region.name}, Kenya. AI-powered market analysis, competitor intelligence, and financial projections in Kenya.`;
  const canonicalUrl = `${SITE_URL}/opportunity/ke/${regionSlug}/${citySlug}/${sectorId}`;
  return {
    title,
    description,
    keywords: [
      `${sector.label.toLowerCase()} business ${city.city}`,
      `start a ${sector.shortLabel.toLowerCase()} business in ${city.city} Kenya`,
      `business opportunity ${city.city} ${region.name}`,
      `best businesses to start in ${city.city}`,
    ],
    robots: { index: true, follow: true },
    alternates: { canonical: canonicalUrl },
    openGraph: { type: 'article', title, description, url: canonicalUrl, siteName: 'BIG — Business Opportunity Intelligence', locale: 'en_KE' },
    twitter: { card: 'summary_large_image', title, description },
  };
}

export default async function KEOpportunityPage({ params }: { params: Promise<{ region: string; city: string; sector: string }> }) {
  const { region: regionSlug, city: citySlug, sector: sectorId } = await params;
  const region = getRegionBySlug(COUNTRY_CODE, regionSlug);
  const city = getCityBySlug(COUNTRY_CODE, regionSlug, citySlug);
  const sector = getSectorById(sectorId);
  ...
  const canonicalUrl = `${SITE_URL}/opportunity/ke/${regionSlug}/${citySlug}/${sectorId}`;
  const ctaUrl = `${APP_URL}?country=KE&region=${region.code}&city=${encodeURIComponent(city.city)}&sector=${sector.apiId}`;

  const jsonLd = [{
    '@context': 'https://schema.org', '@type': 'WebPage',
    name: `${sector.label} Business Opportunities in ${city.city}, ${region.name}, Kenya`,
    url: canonicalUrl,
    breadcrumb: {
      '@type': 'BreadcrumbList',
      itemListElement: [
        { '@type': 'ListItem', position: 1, name: 'Home', item: SITE_URL },
        { '@type': 'ListItem', position: 2, name: 'Kenya', item: `${SITE_URL}/opportunity/ke` },
        { '@type': 'ListItem', position: 3, name: region.name, item: `${SITE_URL}/opportunity/ke/${regionSlug}` },
        { '@type': 'ListItem', position: 4, name: city.city, item: `${SITE_URL}/opportunity/ke/${regionSlug}/${citySlug}` },
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `Is ${sector.label.toLowerCase()} a good business in ${city.city}, Kenya?`,
        acceptedAnswer: { '@type': 'Answer', text: `${city.city}, ${region.name} offers a dynamic market for ${sector.label} businesses. BIG's AI analyzes local market density, demand trends, and financial projections to generate a BIG Score for this specific opportunity in Kenya.` },
      },
      {
        '@type': 'Question',
        name: `How much does it cost to start a ${sector.label.toLowerCase()} business in ${city.city}?`,
        acceptedAnswer: { '@type': 'Answer', text: `Startup costs for ${sector.label} businesses in ${city.city} are in ${country.symbol} and vary by business model. BIG provides detailed financial projections including startup cost ranges and time-to-profitability tailored to the Kenya market.` },
      },
    ],
  }];
    ...
    <>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
        <nav style={{ fontSize: '13px', color: '#9ca3af', marginBottom: '24px' }}>
          ...
          <a href="/opportunity/ke" style={{ color: '#6366f1' }}>Kenya</a> {' → '}
          <a href={`/opportunity/ke/${regionSlug}`} style={{ color: '#6366f1' }}>{region.name}</a> {' → '}
          <a href={`/opportunity/ke/${regionSlug}/${citySlug}`} style={{ color: '#6366f1' }}>{city.city}</a> {' → '}
          {sector.label}
        </nav>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          <h1 style={{ fontSize: '36px', fontWeight: 800, color: '#111827', marginBottom: '12px', lineHeight: 1.2 }}>
            Best {sector.label} Business Opportunities in {city.city}, {region.name} (Kenya)
          </h1>
          <p style={{ fontSize: '16px', color: '#6b7280', maxWidth: 700 }}>
            AI-powered market analysis for {sector.label.toLowerCase()} businesses in {city.city}. Competitor intelligence, financial projections in {country.symbol}, and a 90-day launch plan tailored to Kenya.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '32px' }}>
          <div>
            <section style={{ marginBottom: '40px' }}>
              ...
              {[
                {
                  q: `Is ${sector.label.toLowerCase()} a good business to start in ${city.city}, Kenya?`,
                ...
                },
                {
                  ...
                  a: `Competition in the ${city.city} ${sector.label} market is shaped by local demographics, existing business density, and demand trends unique to the Kenya market. BIG identifies specific gaps and underserved segments.`
                },
                {
                ...
                },
                {
                  ...
                  a: `BIG identifies the highest-scoring opportunities in the ${sector.label} sector for ${city.city} based on local market data. The analysis includes specific business concepts, competitor landscape, and a 90-day launch plan tailored to Kenya.`
                },
              ].map(({ q, a }) => (
            ...
            <section>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px' }}>
                {otherSectors.map(s => (
                  <a key={s.id} href={`/opportunity/ke/${regionSlug}/${citySlug}/${s.id}`}
          ...
          <aside>
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '14px', padding: '20px', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {otherCities.map(c => (
                  <a key={c.slug} href={`/opportunity/ke/${regionSlug}/${c.slug}/${sectorId}`}
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '14px', padding: '20px' }}>
              ...
              <p style={{ fontSize: '13px', color: '#6b7280', lineHeight: 1.6 }}>
                ...
                with Claude AI to generate hyper-local market intelligence for entrepreneurs in Kenya.
